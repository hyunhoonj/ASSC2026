...
def session_body(s, day):
    ...
    label = {"tutorial":"Tutorial","keynote":"Keynote","symposium":"Symposium",
             "concurrent":"Concurrent Session","poster":"Poster Session","special":"Event"}[typ]
...
def build_index(prog):
    ...
    for d in prog["days"]:
        ...
        cards.append('<a class="dcard" href="%s/index.html"><div class="d">%s · %s</div>'
...
def resolve(prog, ref):
    ...
    dm = re.search(r"day\s*([1-4])", low)
    day = "day%s" % dm.group(1) if dm else None
    rest = (low[:dm.start()] + low[dm.end():]) if dm else low  # drop the "dayN" token first
    ...
    tsub=[s for s in prog["sessions"] if low in s["title"].lower()]
